...
from funclite.baselib import tuple_add_elementwise as _tupadd
...
def splitfn(fn):
    path, fn = _path.split(fn)
    name, ext = _path.splitext(fn)
...
def overlay(l_img, s_img, x_offset, y_offset, s_img_alpha_px=(255, 255, 255)):
    ...
    _np.ones(s_img.shape[0:2]) * s_img_alpha_px[0]
    _np.ones(s_img.shape[0:2]) * s_img_alpha_px[1]
    _np.ones(s_img.shape[0:1]) * s_img_alpha_px[2]
...
def nothing(*dummy1, **dummy2):  # noqa
    pass
...
def draw_line(pts, img, line_color=(0, 0, 0), label=False, label_offset_x=10, label_offset_y=10, label_color=(0, 0, 0)):
    ...
    if label:
        for i, pt in enumerate(pts):
            ...
            _cv2.putText(img, lbl, (int(pt[0]) + label_offset_x, int(pt[1]) + label_offset_y), _cv2.FONT_HERSHEY_PLAIN, 0.8, label_color)
    return img
...
def draw_points(pts, img=None, x=None, y=None, join=False, line_color=(0, 0, 0), thickness=1, show_labels=True, label_color=(0, 0, 0), padding=0, add_scale=True, radius=6, plot_centroid=False):
    ...
    if pad > 0:
        ...
        if plot_centroid:
            pt_mean = [pt_mean[0] + pad, pt_mean[1] + pad]  # noqa
    ...
    if join:
        pts_padded = order_points(pts_padded)
        poly_pts = _np.array(pts_padded, dtype='int32')  # noqa
        poly_pts = poly_pts.reshape((-1, 1, 2))
...
def order_points(p):
    ...
    pts = pts.tolist()
    cent = (sum([p[0] for p in pts]) / len(pts), sum([p[1] for p in pts]) / len(pts))  # noqa
    pts.sort(key=lambda v: _math.atan2(v[1] - cent[1], v[0] - cent[0]))  # noqa
    return pts
